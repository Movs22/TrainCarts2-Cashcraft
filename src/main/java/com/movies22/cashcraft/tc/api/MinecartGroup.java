...
public class MinecartGroup {
	...
	private Double targetSpeed = 0.6;
	...
	public MinecartGroup(MetroLine l, String h, int t) {
		this.setLine(l);
		this.setHeadcode(h);
		this._setLength(t);
	}
	 ...
	 * chunksBuffer.add(mm.getEntity().getLocation().getChunk().getX(),
	 * mm.getEntity().getLocation().getChunk().getZ());
	 * }
	 * }
	 * return chunksBuffer;
	 * }
	...
	/*
	 * private void updateChunkInformation(boolean keepChunksLoaded, boolean
	 * isRemoving) {
	 * /*this.chunkArea.refresh(this._spawn.loc.getWorld(),
	 * this.loadChunksBuffer());
	 * if (keepChunksLoaded) {
				...
				if (!m.virtualized) {
					m.getEntity().getPassengers().forEach(r -> {
						if (r instanceof Player) {
							...
							Player p = (Player) r;
							if (reason != null) {
								p.spigot().sendMessage(ChatMessageType.ACTION_BAR, ComponentSerializer
										.parse("{\"text\":\"" + label + "\", \"color\":\"dark_red\",\"bold\":true}"));
							}
			...
			m.getEntity().getPassengers().forEach(e -> {
				if (e instanceof Player) {
					...
					if (r) {
						...
						p.spigot().sendMessage(ChatMessageType.ACTION_BAR, ComponentSerializer
								.parse("{\"text\":\"" + a + "\", \"color\":\"" + this.line.getColour() + "\"}"));
					}
					if (!s) {
	...
	public void virtualize() {
		...
		//if (this.head().currentSpeed > 0.05) {
			...
			for (int i = 1; i < this.members.size(); i++) {
				...
				// marks cart as removed (so its skipped by the controller) and removes it from
				// the MemberStore's cache (so it isn't cached with the wrong UUID)
				// SKIPS head cart (i=0)
				mm.spawned = false;
	...
	public void unVirtualize(Boolean offset) {
		this.head().spawned = false;
		this.head().load();
		for (int i = 1; i < this.members.size(); i++) {
			MinecartMember mm = this.members.get(i);
